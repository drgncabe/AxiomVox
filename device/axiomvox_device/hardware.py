...
class HardwareProbe:
    ...
    def collect(self) -> HardwareStatus:
        ...
        whisplay_button = self._probe_input("whisplay")
        pisugar = self._probe_pisugar()
        battery = self._read_battery_percentage()
        pisugar_button = self._probe_input("pisugar")
    ...
    def _read_battery_percentage(self) -> int | None:
        ...
        if pisugar_battery:
            try:
                return max(0, min(100, round(float(pisugar_battery))))
            except ValueError:
                pass
